// src/components/TopNavbarClient.tsx
"use client";
...
/* ============================================================
   Helper para agregar una versión (?v=timestamp) y evitar cache
   ============================================================ */
function withVersion(url: string, v?: string | number) {
  if (!v) return url;
  try {
    const u = new URL(
      url,
...
export default function TopNavbarClient({
  ...
  const finalSrc = useMemo(() => {
    const raw = avatarUrl || "/images/avatar-default.png";
        ...
        <div className="mx-auto flex h-16 md:h-24 max-w-7xl items-center justify-between px-4 sm:px-6 md:px-8 lg:px-16">
          <div className="flex items-center gap-4 md:gap-12">
            ...
            <nav className="hidden lg:flex gap-6 xl:gap-8 text-sm lg:text-base font-medium">
              <Link href="/dashboard/home" className="hover:text-[#FFBD00] transition-colors duration-200 py-2">
                HOME
              </Link>
              <Link href="/dashboard/inventory" className="hover:text-[#FFBD00] transition-colors duration-200 py-2">
                INVENTARIO
              </Link>
              <Link href="/dashboard/support" className="hover:text-[#FFBD00] transition-colors duration-200 py-2">
                CONTÁCTANOS
              </Link>
            </nav>
          </div>

          {/* Bloque Bienvenida + Avatar */}
          <div className="flex items-center gap-3 md:gap-6 max-w-[60%] md:max-w-none">
            {!!nombre && (
              <>
                <div className="hidden sm:flex items-center gap-3 md:gap-4">
                  {/* AVATAR con cache-buster y fallback */}
                  <div className="relative h-8 w-8 md:h-12 md:w-12 rounded-full overflow-hidden ring-2 ring-white/15 bg-[#0F2744]">
                    <Image
                      key={finalSrc} // fuerza re-render al cambiar ?v=
                      src={finalSrc}
                      alt="Avatar empresa"
                      fill
                      sizes="48px"
                      ...
                      onError={(e) => {
                        ...
                        if (!img.src.includes("/images/avatar-default.png")) {
        ...
        <div className="fixed inset-0 z-[60] lg:hidden" role="dialog" aria-modal="true">
          ...
          <aside className="absolute left-0 top-0 h-full w-80 max-w-[85vw] bg-white text-gray-900 shadow-2xl p-4 overflow-y-auto flex flex-col">
            ...
            <div className="bg-gray-50 rounded-lg border p-3 mb-4">
              ...
              <p className="text-xs text-gray-500">RUC: {ruc || "N/A"}</p>
